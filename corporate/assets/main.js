...
(function () {
  ...
  if (hscroll && hTrack) {
    ...
    function updateHScroll() {
      ...
      if (distance <= 0 || maxShift <= 0) { hTrack.style.transform = ""; return; }
      var progress = Math.min(Math.max(-rect.top / distance, 0), 1);
      hTrack.style.transform = "translate3d(" + (-progress * maxShift) + "px,0,0)";
    }
    function onHScroll() {
      if (!ticking) { ticking = true; requestAnimationFrame(updateHScroll); }
    }
  ...
  var specialties = [
    ...
    "Psiquiatras", "Médicos", "Terapeutas integrativos"
  ];
  var specTrack = document.getElementById("specTrack");
  if (specTrack) {
    var specHTML = specialties.map(function (s) {
  ...
  var clients = [
    { name: "BeTalent", src: "https://www.souplenus.com.br/_nuxt/img/betalent-logo.ae67c25.jpg" },
    { name: "Inove Casa", src: "https://www.souplenus.com.br/_nuxt/img/inove-casa-logo.ff7247f.jpeg" },
    { name: "The Place English School", src: "https://www.souplenus.com.br/_nuxt/img/the-place-logo.bc97f55.png" },
    { name: "Universidade Federal de Pelotas", src: "https://www.souplenus.com.br/_nuxt/img/ufpel-logo.4569345.jpeg" }
  ];
  var logosTrack = document.getElementById("logosTrack");
  if (logosTrack) {
    var logoHTML = clients.map(function (c) {
      return '<span class="client-logo"><img src="' + c.src + '" alt="' + c.name + '" /></span>';
    }).join("");
  ...
  if (form) {
    function validateField(field) {
      ...
      if (!val) ok = false;
      else if (input.type === "email") ok = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(val);
